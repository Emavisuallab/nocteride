...
export default function DriverMap() {
    ...
    if (!driverMarkerRef.current) {
      ...
      el.innerHTML = `
        <div style="position:relative;width:24px;height:24px;">
          ...
          <div style="position:absolute;inset:-6px;background:rgba(155,127,232,0.25);border-radius:50%;animation:drvPulse 2s ease-in-out infinite;"></div>
        </div>`
      if (!document.getElementById('drv-pulse-style')) {
        const s = document.createElement('style')
        s.id = 'drv-pulse-style'
        s.textContent = '@keyframes drvPulse{0%,100%{transform:scale(1);opacity:.25}50%{transform:scale(1.5);opacity:0}}'
      ...
      async (pos) => {
        ...
        if (now - lastSentRef.current >= 30000) {
          ...
          await supabase.from('location_updates').insert({
            tracking_session_id: sessionId, lat: loc.lat, lng: loc.lng,
          })
        }
      },
      (err) => {
  ...
  async function handleStartTrip() {
    if (!serviceId) return
    if (gpsStatus !== 'active') { toast('Necesitas GPS activo para iniciar', 'error'); return }
  ...
  const gpsColors: Record<GpsStatus, string> = {
    checking: '#8888A8', prompting: '#F0A070', active: '#4CAF82', denied: '#EF4444', unavailable: '#EF4444',
  }
  const gpsLabels: Record<GpsStatus, string> = {
    checking: 'Verificando GPS...', prompting: 'Activando GPS...', active: 'GPS activo',
    denied: 'GPS denegado', unavailable: 'GPS no disponible',
  }
      ...
      <div className="absolute bottom-4 left-4 right-4 z-10">
        <div className="bg-[#1A1A2E] border border-[rgba(155,127,232,0.15)] rounded-3xl p-4">
          ...
          <div className="flex items-center gap-2 mb-2">
            <div className={`w-2.5 h-2.5 rounded-full ${gpsStatus === 'active' ? 'animate-pulse' : ''}`}
              style={{ background: gpsColors[gpsStatus] }} />
          ...
          {(gpsStatus === 'denied' || gpsStatus === 'unavailable') && (
            <div className="mb-3 p-3 rounded-xl bg-[rgba(239,68,68,0.1)] border border-[rgba(239,68,68,0.2)]">
              <p className="text-sm text-[#EF4444] font-medium mb-1">
                {gpsStatus === 'denied' ? 'Permiso de ubicación denegado' : 'GPS no disponible'}
              </p>
              <p className="text-xs text-[#8888A8]">
                {gpsStatus === 'denied'
                  ? 'Abre configuración del navegador → Permisos → Ubicación → Permitir. Luego recarga.'
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {myLocation && (
                    <p className="text-xs text-[#8888A8]">{myLocation.lat.toFixed(5)}, {myLocation.lng.toFixed(5)}</p>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[#F0F0FF]">Viaje listo — listo para salir</span>
              </div>
              <Button className="w-full h-14 text-lg" size="lg" onClick={handleStartTrip}
                loading={actionLoading} disabled={gpsStatus !== 'active'}>
                Ya salí
              </Button>
            </div>
          ) : (
